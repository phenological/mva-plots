...
```
```{r}
lo<-ropls::opls(x = BIOspcglyc[,1:8], y = BIOspcglyc[,"covid_status"], predI = 1, orthoI = 0, crossvalI = 10, permI = 50)
lopredict<-ropls::predict(object = lo, newdata=aaData)


#1 make the model

lo<-oplsda(X=BIOspcglyc[,1:8], Y=BIOspcglyc$covid_status, type = "PLS", optns=list(predI = 3, permI= 100, scaleC="none"))
lot<-oplsda(X=BIOspcglyc[,1:8], Y= as.factor(BIOspcglyc$covid_status), type = "OPLS", optns=list(predI = 3, permI= 100))

#2 predict with newdata 
lotpred <-oplsdaPredict(model = lo, newdata = aaData)
lot3pred <-oplsdaPredict(model = lot, newdata = BIOspcglyc[,1:8], optns=list(real = as.factor(BIOspcglyc$covid_status)))

#mtcars test
il<- oplsda(X=mtcars[,1:7], Y=mtcars[,8], type = "OPLS", optns=list(scaleC="none"))
ilpred <-oplsdaPredict(model = il, newdata = mtcars[,1:7])

il<- oplsda(X=iris[,1:3], Y=iris[,4], type = "PLS", optns=list(scaleC="none"))
ilpred <-oplsdaPredict(model = il, newdata = iris[,1:3])

plotScores(model=lot, optns = list(ellipse = "hotellings", discretePalette = c("covid" = "purple", "control" = "orange")))

plotScores(model=a, optns=list(color = BIOspcglyc$age ,ellipse = "normal"))

plotLoadings(model = a, optns = list())
```

```{r}
lot2 <-eruptionPlot(model = lot, optns=list(y="pval", x="fc", factor = BIOspcglyc$covid_status, continuousPalette = c("#B2182B", "#D6604D", "#F4A582", "#FDDBC7", "#D1E5F0", "#92C5DE", "#4393C3", "#2166AC")))

lot@suppLs[["yMCN"]]
#append to ropls object 
#slot(lo,"original") <- BIOspcglyc[,1:8]
b<-metabom8::opls(X = BIOspcglyc[,1:8], Y = BIOspcglyc$covid_status, center = TRUE, scale = TRUE)

lot@suppLs[["original"]] <-append(x = data.frame(), values = originalX)

as.data.frame(lo@suppLs[["original"]])

optns<-list(x="pval", y="cd")
...
maiC <- paste0("pR2Y = ",
                   ...
                   ", pQ2 = ",
                   lot@summaryDF[, "pQ2"])
 plot(c(min(lot@suppLs[["permMN"]][, "sim"]), 1),
       c(min(lot@suppLs[["permMN"]][, c("R2Y(cum)", "Q2(cum)")]), 1),
       main = maiC,
       type = "n",
       xlab = expression(Similarity(bold(y), bold(y[perm]))),
       ylab = "")
 points(lot@suppLs[["permMN"]][, "sim"], lot@suppLs[["permMN"]][, "Q2(cum)"],
           col = "black",
           pch = 18)
    abline(h = lot@suppLs[["permMN"]][1, "Q2(cum)"],
           col = "black")
    points(lot@suppLs[["permMN"]][, "sim"], lot@suppLs[["permMN"]][, "R2Y(cum)"],
           col = "grey",
           pch = 18)
    abline(h = lot@suppLs[["permMN"]][1, "R2Y(cum)"],
           col = "grey")
    .plotLegendF(c("R2Y", "Q2Y"),
                 "bottomright",
                 colVc = c("grey", "black"))

df <- as.data.frame(lot@suppLs[["permMN"]])

ggplot() + 
  geom_point(data=df[-1,], aes(x =`sim`, y =`Q2(cum)` ), color="red", alpha=0.4,  size = 3)+
  geom_point(data=df[-1,], aes(x =`sim`, y =`R2Y(cum)` ), color="blue", alpha=0.4,  size = 3)+
  geom_point(data=df[1,], aes(x =`sim`, y =`Q2(cum)`), color="red", size = 3 )+
  geom_point(data=df[1,], aes(x =`sim`, y =`R2Y(cum)`), color="blue", size = 3) +
  geom_hline(yintercept = df[1, "Q2(cum)"], colour = "red")+
   geom_hline(yintercept = df[1, "R2Y(cum)"], colour = "blue")+
...
permutationPlot(model = lot, optns=list(shape="square", colorQ="#FF5500", size = 5))

Qdf<-  df[,c("Q2(cum)", "sim")] 
Qdf <-data.frame(append(Qdf, c(st='Q2'), after=2))
Rdf <- df[,c("R2Y(cum)", "sim")]
Rdf <-data.frame(append(Rdf, c(st='R2Y'), after=2))
dfmlt<- rbind(Qdf, Rdf)

colors <- c("Q2(cum)" = "blue", "R2Y(cum)" = "red")

ggplot(df, aes(x = sim)) +
    geom_point(data= df[1,],aes(y = `Q2(cum)`, color = "Q2(cum)"), size = 1.5) +
    geom_point(data= df[1,],aes(y = `R2Y(cum)`, color = "R2Y(cum)"), size = 3) +
    geom_point(data= df[-1,],aes(y = `Q2(cum)`, color = "Q2(cum)"), size = 1.5, alpha = 0.4) +
    geom_point(data= df[-1,],aes(y = `R2Y(cum)`, color = "R2Y(cum)"), size = 3) +
    geom_hline(yintercept = df[1, "Q2(cum)"], colour = "blue") +
   geom_hline(yintercept = df[1, "R2Y(cum)"], colour = "red") +
    labs(x = "Year",
         y = "(%)",
         color = "Legend") +
...
a1 <- PCA(df3[,1:112], rank = 20)
##plotScores(pca,optns=list(color=type, ellipse="color", plotTitle="2022 All Wines NMR PCA"))

#PCA tests
 plotScores(model=a, optns=list(color=BIOspcglyc$age, ellipse = "color", shape=BIOspcglyc$sex, outlierLabels=row.names(BIOspcglyc)))

plotScores(model = a, optns = list(color = BIOspcglyc$covid_status, discretePalette = c("covid" = "purple", "control" = "orange"), ellipse = "color", PCi = 2, PCj = 3, ci = 0.9, theme = theme(legend.position = "left", text=element_text(size=5))))
 
#2
fff<- plotScores(model=a, optns=list(gt = theme(legend.position = "left", text=element_text(size=5)), color=BIOspcglyc$covid_status, ellipse="color", shape=BIOspcglyc$sex, outlierLabels=row.names(BIOspcglyc)))
fff[["plots"]][["pcaGrid"]][1,1]

#ropls objects
fff <-plotScores(model = a, optns = list(color = BIOspcglyc$covid_status, discretePalette = c("covid" = "purple", "control" = "orange"), ellipse = "color", PCi = 1, PCj = 2,  annotation = BIOspcglyc[,10:15], outlierLabels = BIOspcglyc$covid_status))

#2
fff<- plotLoadings(model= a)
fff[["plots"]][["plotLoadingGrid"]]

eruptionPlot(model = lot, optns=list(factor=BIOspcglyc$covid_status, plotTitle = "Biogune SpcGlyc Eruption "))
...
b <- PCA(data = SER_NOESY_data[,-38623], rank = 3)
plotScores(model=b, optns=list(color=(SER_NOESY_meta$Age)))
```

```{r}
df2 <- as.data.frame(cbind(lot@scoreMN, lot@orthoScoreMN))
df2 <-cbind(df2, lot@suppLs[["yMCN"]])
...
PCi = 1
PCj = 2
    ggplot(data = df, aes(x=df[,PCi], y=df[,PCj])) + 
  geom_point(aes(color=BIOspcglyc$age, shape=BIOspcglyc$sex))+ 
  # geom_label(data=df[idx,],
  #            aes(x=p1, y = p2,label=row.names(df[idx,])))+
 ...
 palette <- scale_color_brewer(palette = "Set2")
```
...
g <- metabom8::opls(X=BIOspcglyc[,1:8], Y=BIOspcglyc[,"covid_status"])
...
lot2<-ropls::opls(x = BIOspcglyc[,1:8], y = BIOspcglyc$covid_status, predI = 1, orthoI = NA, permI=20)
```
