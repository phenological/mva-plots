...
```{r}
#1 make the model
lo<-oplsda(X=BIOspcglyc[,1:8], Y=BIOspcglyc$covid_status, type = "OPLS", optns=list(predI = 3, permI= 100))

#2 predict with newdata 
lot <-oplsdaPredict(object = lo, newdata = aaData)
...
lo<-ropls::opls(x = BIOspcglyc[,1:8], y = BIOspcglyc$covid_status, predI = NA, orthoI = 0, permI=20, subset = "odd")
```
