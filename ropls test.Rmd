...
plotScores(model=lot, optns = list(ellipse = "color", discretePalette = c("covid" = "purple", "control" = "orange")))

plotScores(model=a, optns=list(ellipse = "hotellings"))
...
#2
fff<- plotScores(model=a, optns=list(color=BIOspcglyc$age, continuousPalette = viridis, ellipse="hotellings", shape=BIOspcglyc$sex, outlierLabels=row.names(BIOspcglyc)))
fff[["plots"]][["pcaGrid"]][1,1]

#ropls objects
fff <-plotScores(model = a, optns = list(color = BIOspcglyc$covid_status, discretePalette = c("covid" = "purple", "control" = "orange"),ellipse = "t", outlierLabels=BIOspcglyc$covid_status, annotation = BIOspcglyc[,10:12]))

#2
fff<- plotLoadings(model= a)
fff[["plots"]][["plotLoadingGrid"]]

eruptionPlot(model = a, optns=list(factor=BIOspcglyc$covid_status))
```
