...
mod<-PCA(data = X, annotation = NOESY_ANN,rank = 2)
plotScores(model = mod, optns = list(color = factor(NOESY_ANN$sampleType)))

a <- PCA(data = SER_NOESY_data[,-38623], annotation = SER_NOESY_meta, rank = 2, plot = FALSE)
plotScores(model  = a, optns= list(color=factor(SER_NOESY_meta$device)))
```
