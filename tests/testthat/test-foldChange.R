...
test_that("can use with dataframe with more than 2 groups",{
  ...
  log2fcdf<- foldChange(model = result2, optns = list(factor = data$fact, control = "control"))
  ...
  #        df <- model
  #        df$factor <- as.numeric(relevel(factor(data$fact), ref = optns$control))
  #        # Initialize an empty data frame to store log2fc values
  ...
  #      }
  #
  #      #make ref table for what each factor was originally????? use optns$factor and df$factor to make the list
  #
  ...
  #
  #
  # foldChange(model = result2, optns = list(factor = data$fact, control = "control"))
