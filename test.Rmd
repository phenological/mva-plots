...
```{r}
#testthat, ggplot2, GGally, roxygen2, stats, ggrepel, dplyr??, sp??(included), ggpubr??
...
a <- pcResults(BIOspcglyc[,1:8], annotation = BIOspcglyc[,-1:-8], plot = FALSE)
a1 <- pcResults(df3[,1:112], annotation = df3[,-1:-112])
...
# 
# 
# opt=list(colourCoding= "covid_status")

#2) 
fff<- plotScores(model=a, optns=list(colourCoding=BIOspcglyc[,"age"], shapeCoding=BIOspcglyc[,"sex"], ellipse="hotellings", outlierLabels=row.names(BIOspcglyc)))
fff[["plots"]][["pcaGrid"]]
...
b<-plotScores(model=a, optns=list(colourCoding=iris[,5], colourTitle="species", ellipse="T", outlierLabels=rownames(iris), thresh=3))
b[["plots"]][["pcaGrid"]]
