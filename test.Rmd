...
```{r}
#ggplot2, prcomp, GGally, roxygen2, stats, BioGenerics, dplyr??, sp??(included), ggpubr??
...
a <- pcResults(BIOspcglyc[,1:8], annotation = BIOspcglyc[,-1:-8], rank = 3)
...
#2) 
fff<- plotscores(model=a, optns=list(colourCoding=BIOspcglyc[,"covid_status"], sizeCoding=3, alphaCoding=BIOspcglyc[,"age"], shapeCoding=BIOspcglyc[,"sex"], ellipse="normal", confidenceInterval=0.90, thresh=3))
fff[["plots"]][["pcaGrid"]]

#2)
ffff<- plotLoadings(model= a)
ffff[["plots"]][["plotLoadingGrid"]]
```
...
idx<- which(BIOspcglyc[,"covid_status"]=="covid")
 p <- ggplot(data=a$pcdf, aes(x=a$pcdf$PC4, y=a$pcdf$PC5)) +
  geom_point(aes(shape=a$pcdf$sex))+
  #scale_color_brewer(palette = "Set2")+
  stat_ellipse( type = "t", geom = "polygon", fill = "gray", level = 0.95, alpha = .5, linetype = 2)
