...
#' title of the plot.
#' @param thresh A parameter for the \code{optns} list. A numeric for the number
#' of PCAs to display in the grid. The default is calculated in the PCA function.
#' @param color A parameter for the \code{optns} list. Either a column from the
#' data frame or a character of the color desired (example "blue"). Default
#' color is "black". When using on a ropls object, it must match the quantitative
#' data type (discrete or continuous).
#' @param shape A parameter for the \code{optns} list. Either a column from the
#' data frame (must be discrete) or a character of the shape desired. Default
#' shape is "circle".
#' @param size A parameter for the \code{optns} list. Either a column from the
#' data frame or a numeric of the size desired. Default size is 3.
#' @param alpha A parameter for the \code{optns} list. Either a column from the
#' data frame or a numeric of the alpha desired. Default size is 0.5.
#' @param discretePalette A parameter for the \code{optns} list. Color palette
#' for discrete values, you can assign colors to specific factors, example:
#' discretePalette = c("control" = "purple", "treatment" = "orange"). Or supply
#' a concatenated list, example (and the default): discretePalette = c("#B2182B",
...
#' set theme = theme(legend.position = "left", text=element_text(size=5)) in optns.
#' @param colorTitle A parameter for the \code{optns} list. A character of the
#' desired color legend title when \code{color} is a variable. No color legend
#' will appear if \code{color} is set to a simple aesthetic such as "green".
#' Default "color".
#' @param shapeTitle A parameter for the \code{optns} list. A character of the
#' desired shape legend title when \code{shape} is a variable. No shape legend
#' will appear if \code{shape} is set to a simple aesthetic such as "square".
#' Default "Shape".
#' @param sizeTitle A parameter for the \code{optns} list. A character of the
#' desired shape legend title when \code{size} is a variable. No size legend
#' will appear if \code{size} is set to a simple aesthetic such as 2. Default "Size".
#' @param alphaTitle A parameter for the \code{optns} list. A character of the
#' desired alpha legend title when \code{alpha} is a variable. No size legend
#' will appear if \code{alpha} is set to a simple aesthetic such as 0.3. Default
#' "Alpha".
#' @param ellipse A parameter for the \code{optns} list. A character or either
#' "color", "hotellings", "t", or "normal" depending on desired method of
#' calculation. If using color, a discrete variable must be supplied to color.
#' @param ci A parameter for the \code{optns} list. Set your own limit for
#' ellipses drawn. The default is ci = 0.95 (95% confidence interval).
#' @param outlierLabels A parameter for the \code{optns} list. Only compatible
#' with ellipse set to hotellings, T or normal. For ropls object, supply
#' "outlierLabels" and rownames will appear. For PCA modek, supply a column from
#' the data frame to label outliers. You can set it to, for example,
#' outlierLabels = row.names(iris) to identify the outlier position in your dataframe.
#' @param annotation A parameter for the \code{optns} list when supplying PCA
#' object. Supply the columns you'd like listed for the outliers, otherwise only
#' that listed for outlierLabels will be listed.
