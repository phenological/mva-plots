...
#'   \itemize{
#'     \item{factor} {An object the same length as the data used to build the
#'      model that must be a two factor variable such as treatment and control. 
#'      Mandatory when supplying a PCA object. For oplsda the response
#'      variable is used by default.}
#'     \item{control} {Factor level to be interpreted as control. Defaults
#'     to the first level.} 
...
#'      \item{significance} {Points with p < significance are left unlabeled. 
#'      If not provided, all points are labeled.}
#'   }
...
eruptionPlot <- function(model, optns = list()){
  ...
  if("significance" %in% names(optns)){
    optns$significance <- abs(log10(1 - optns$significance))
  } else{
    optns$significance <- 0
  }
  ...
  if("external" %in% names(optns)){
    ...
    pval <- apply(dF[,1:(ncol(dF)-1)],2,function(v){
      kruskal.test(v,dF[,"factor"])$p.value
    })
    # pval<-list()
  ...
  #eruption data frame
  ed<-cbind(cd, fc, pvalRescaled, pval, pcLoadings, id, corr)
  ...
  ############plot#########
  sigfi <- ed$pval >= optns$significance
  eruptionPlot <- ggplot(data = ed, aes(x = x,
    ...
    geom_label_repel(aes(label = ifelse(sigfi,id,NA)),
                     colour = "black",
  ...
  #abernal: Won't this legend be unsynced from the actual plot colors?
